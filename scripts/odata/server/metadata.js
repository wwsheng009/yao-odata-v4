...
function createMetadataJSON(server) {
  ...
  const propNames = utils_1
    .getAllPropertyNames(server.prototype)
  ...
  const resolveTypeDefinition = (
    ...
    if (!defName) {
      defName =
        defType["@odata.type"] ||
        (defType["prototype"] && defType["prototype"]["@odata.type"]) ||
        prop;
      ...
      defDefinition = {
        name: defName,
        underlyingType:
          getTypeName(defType, undefined, server.container) || "System.Object",
      };
    } else {
      if (defName.indexOf(".") > 0) {
        defNamespace = defName.slice(0, defName.lastIndexOf("."));
      ...
      defDefinition = {
        name: defName,
        underlyingType:
          getTypeName(containerType, defName, server.container) ||
          "System.Object",
      };
    }
    let defSchema = definition.dataServices.schema.filter(
      (schema) => schema.namespace == defNamespace
    )[0];
    if (!defSchema) {
      defSchema = {
  ...
  const resolveEnumType = (
    ...
    let enumNamespace =
      odata.getNamespace(containerType, server.container.resolve(enumType)) ||
      namespace;
    let enumDefinition;
    if (!enumName) {
      ...
      enumDefinition = {
        ...
        underlyingType:
          ...
          "Edm.Int32",
        isFlags:
          Edm.isFlags(containerType, `${enumNamespace}.${enumName}`) ||
          Edm.isFlags(containerType, enumName),
        member: [],
        annotation: Edm.getAnnotations(enumType),
      };
    }
    let enumSchema = definition.dataServices.schema.filter(
      (schema) => schema.namespace == enumNamespace
    )[0];
    if (!enumSchema) {
      enumSchema = {
  ...
  const resolveOperation = (
    ...
    if (Edm.isTypeDefinition(elementType, prop)) {
      ...
      let definitionContainer = Edm.isEntityType(type)
        ? "entityType"
        : "complexType";
      let resolvedType = resolveType(type, elementType, prop);
      if (
        resolvedType &&
        !resolvedType.schema[definitionContainer].filter(
          (et) => et.name == resolvedType.definition.name
        )[0]
      ) {
    ...
    let parameters = Edm.getParameters(elementType, prop).map((p) => {
      ...
      if (typeof param.type != "string") {
        let resolvedContainerType =
          server.container.resolve(param.type) || param.type.name;
        if (Edm.isTypeDefinition(param.type)) {
          param.type = resolveTypeDefinition(
            containerType,
            resolvedContainerType,
            namespace,
            Edm.getType,
            () => {
              return Edm.getTypeName(
                containerType,
                resolvedContainerType,
                server.container
              );
          ...
          if (typeof param.type == "function") {
            let definitionContainer = Edm.isEntityType(param.type)
              ? "entityType"
              : "complexType";
            let resolvedType = resolveType(
              ...
              prop,
              undefined,
              resolvedContainerType.indexOf(".") > 0
                ? resolvedContainerType
                : `${
            ...
            if (
              ...
              resolvedType.schema[definitionContainer].push(
          ...
          if (resolvedContainerType.indexOf(".") > 0) {
            ...
            param.type = `${
              ...
              odata.getNamespace(containerType, resolvedContainerType) ||
              namespace
            }.${resolvedContainerType}`;
          }
        }
  ...
  const resolveImport = (operationSchema, i, definition, type) => {
    ...
    let entityContainer = operationSchema.entityContainer.find(
      (ec) => ec.name == containerName
    );
    if (!entityContainer) {
      entityContainer = {
        name: containerName,
  ...
  const resolveType = (elementType, parent, prop, baseType, paramTypeName) => {
    ...
    if (!elementType.namespace)
      elementType.namespace = server.container.resolve(elementType)
        ? containerType.namespace || parent.namespace
        : parent.namespace;
    let typeName =
      paramTypeName ||
      Edm.getTypeName(parent, prop, server.container) ||
      elementType.name;
    if (typeName.indexOf("Collection") == 0) typeName = elementType.name;
    let typeDefinition = {
      name: typeName.split(".").pop(),
      baseType: baseType
        ? (baseType.namespace || server.namespace) + "." + baseType.name
        : undefined,
      key: [],
      property: [],
      navigationProperty: [],
    ...
    let namespace =
      elementType.namespace || parent.namespace || server.namespace;
    let typeSchema = definition.dataServices.schema.filter(
    ...
    properties.forEach((prop) => {
      ...
      if (Edm.isTypeDefinition(elementType, prop)) {
        propertyDefinition.type = resolveTypeDefinition(
          elementType,
          prop,
          namespace
        );
        ...
        if (typeof type == "function") {
          let definitionContainer = Edm.isEntityType(elementType, prop)
            ? "entityType"
            : "complexType";
          let resolvedType = resolveType(type, elementType, prop);
          if (
    ...
    Object.keys(operations).forEach((operation) => {
      let namespace =
        operations[operation].prototype[operation].namespace ||
        elementType.namespace ||
        parent.namespace ||
        server.namespace;
      let elementTypeNamespace =
        elementType.namespace || parent.namespace || server.namespace;
      let operationSchema = definition.dataServices.schema.filter(
        (schema) => schema.namespace == namespace
      )[0];
      if (!operationSchema) {
        operationSchema = {
      ...
      if (Edm.isFunction(operations[operation], operation)) {
        operationSchema.function.push(
          resolveOperation(
            ...
            namespace,
            true,
            `${elementTypeNamespace}.${elementType.name}`
          )
        );
      }
    ...
    if (
      ...
      typeDefinition.baseType =
        (baseType.namespace || server.namespace) + "." + baseType.name;
      let definitionContainer = Edm.isEntityType(baseType)
        ? "entityType"
        : "complexType";
      if (
        resolvedType &&
    ...
    children.forEach((child) => {
      ...
      let definitionContainer = Edm.isEntityType(child)
        ? "entityType"
        : "complexType";
      if (
        resolvedType &&
        !resolvedType.schema.entityType.filter(
          (et) => et.name == resolvedType.definition.name
        )[0]
      ) {
        resolvedType.schema[definitionContainer].push(resolvedType.definition);
  ...
  propNames.forEach((i) => {
    if (i != "$metadata" && server.prototype[i]) {
      if (
        server.prototype[i].prototype instanceof controller_1.ODataController
      ) {
        ...
        let ctrlSchema = definition.dataServices.schema.find(
          (schema) => schema.namespace == ctrl.namespace
        );
        if (!ctrlSchema) {
          ctrlSchema = {
        ...
        for (var es in entitySets) {
          if (entitySets[es] == ctrl) {
            let entityContainer = ctrlSchema.entityContainer.find(
              (ec) => ec.name == containerName
            );
            if (!entityContainer) {
              entityContainer = {
            ...
            entityContainer.entitySet.push({
              name: es,
              entityType:
                Edm.getTypeName(server, i, server.container) ||
                (elementType.namespace || server.namespace) +
                  "." +
                  elementType.name,
            });
          }
  ...
  propNames.forEach((i) => {
    if (i != "$metadata" && server.prototype[i]) {
      let operationNamespace =
        server.prototype[i].namespace || server.namespace;
      let operationSchema = definition.dataServices.schema.filter(
        (schema) => schema.namespace == operationNamespace
      )[0];
      if (!operationSchema) {
        operationSchema = {
      ...
      if (Edm.isActionImport(server, i)) {
        resolveImport(
          operationSchema,
          i,
          resolveOperation(server, i, operationNamespace),
          "action"
        );
      }
  ...
  definition.dataServices.schema = definition.dataServices.schema.sort((a, b) =>
    a.namespace.localeCompare(b.namespace)
  );
  definition.dataServices.schema.forEach((schema) => {
    schema.typeDefinition = schema.typeDefinition.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
    schema.enumType = schema.enumType.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
    schema.entityType = schema.entityType.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
    schema.complexType = schema.complexType.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
    schema.entityContainer = schema.entityContainer.sort((a, b) =>
      a.name.localeCompare(b.name)
    );
  });
  return definition;
