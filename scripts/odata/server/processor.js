...
const { TokenType } = require("./lib/parser");
...
const createODataContext = function (
  context,
  entitySets,
  server,
  resourcePath,
  ...
  resourcePath.navigation.forEach((baseResource, i) => {
    ...
    let selectContextPart =
      i == resourcePath.navigation.length - 1 ? selectContext : "";
    if (next && next.type == TokenType.RefExpression) return;
    if (
      baseResource.type == TokenType.QualifiedEntityTypeName ||
      baseResource.type == TokenType.QualifiedComplexTypeName
    ) {
    ...
    if (baseResource.type == TokenType.EntitySetName) {
      ...
      prevType = baseResource.key
        ? entitySets[baseResource.name].prototype.elementType
        : entitySets[baseResource.name];
      odataContext += baseResource.name;
      odataContext += selectContextPart;
      if (
        baseResource.key &&
        resourcePath.navigation.indexOf(baseResource) ==
          resourcePath.navigation.length - 1
      )
      ...
      if (baseResource.key) {
        if (baseResource.key.length > 1) {
          return (odataContext +=
            "(" +
            baseResource.key
              .map((key) => `${key.name}=${decodeURIComponent(key.raw)}`)
              .join(",") +
            ")");
        } else {
          return (odataContext +=
            "(" + decodeURIComponent(baseResource.key[0].raw) + ")");
        }
      }
    } else if (
      getResourcePartFunction(baseResource.type) &&
      !(baseResource.name in expCalls)
    ) {
      odataContext = "";
      if (prevResource) {
        ...
        let returnType = Edm.getReturnType(
          target,
          propertyKey,
          server.container
        );
        let returnTypeName = Edm.getReturnTypeName(
          target,
          propertyKey,
          server.container
        );
        if (typeof returnType == "function") {
          ...
          returnType = entitySet
            ? entitySet +
              (returnTypeName.indexOf("Collection") == 0
                ? selectContextPart
                : selectContextPart + "/$entity")
            : returnTypeName;
    ...
    if (baseResource.type == TokenType.EntityNavigationProperty) {
      ...
      return entitySet
        ? (odataContext = entitySet + selectContextPart + "/$entity")
        : (odataContext += "/" + baseResource.name);
    }
    if (
...
const ODataRequestResult = {
  ...
  put: (result, contentType) => {
    return (
      result ? result_1.ODataResult.Created : result_1.ODataResult.NoContent
    )(result, contentType);
  },
...
const expCalls = {
  ...
  $value: function (processor) {
    ...
    try {
      let prevPart =
        processor.resourcePath.navigation[
          processor.resourcePath.navigation.length - 2
        ];
      let fn = odata.findODataMethod(
        processor.ctrl,
        `${processor.method}/${prevPart.name}/$value`,
        prevPart.key || []
      );
      if (
        !fn &&
        typeof this.elementType == "function" &&
        Edm.isMediaEntity(this.elementType)
      ) {
        fn = odata.findODataMethod(
          processor.ctrl,
      ...
      if (fn) {
        ...
        if (
          fnDesc.key.length == 1 &&
          prevPart.key.length == 1 &&
          fnDesc.key[0].to != prevPart.key[0].name
        ) {
        ...
        if (
          prevPart.type == "PrimitiveProperty" ||
          prevPart.type == "PrimitiveKeyProperty"
        )
  ...
  $ref: function (processor) {
    ...
    try {
      ...
      let routePart =
        processor.resourcePath.navigation[
          processor.resourcePath.navigation.length - 3
        ];
      let fn = odata.findODataMethod(
        processor.prevCtrl,
        processor.method + "/" + prevPart.name + "/$ref",
        routePart.key || []
      );
      ...
      if (!fn) throw new error_1.ResourceNotFoundError();
      let linkUrl = (
        processor.resourcePath.id ||
        (processor.body || {})["@odata.id"] ||
        ""
      ).replace(getODataRoot(processor.context), "");
      ...
      if (linkUrl) {
        ...
        linkPath = new visitor_1.ResourcePathVisitor(
          processor.serverType,
          processor.entitySets
        ).Visit(linkAst);
      ...
      if (
        fnDesc.key.length == 1 &&
        routePart.key.length == 1 &&
        fnDesc.key[0].to != routePart.key[0].name
      ) {
      ...
      if (
        fnDesc.link.length == 1 &&
        linkPart.key.length == 1 &&
        fnDesc.link[0].to != linkPart.key[0].name
      ) {
...
(function (ODataGeneratorHandlers) {
  ...
  (ODataGeneratorHandlers =
    exports.ODataGeneratorHandlers || (exports.ODataGeneratorHandlers = {}))
);
...
(function (ODataMetadataType) {
  ...
  (ODataMetadataType =
    exports.ODataMetadataType || (exports.ODataMetadataType = {}))
);
class ODataProcessor {
  constructor(context, server, options) {
    ...
    let ast = this.serverType.parser.odataUri(context.url, {
      metadata: this.serverType.$metadata().edmx,
    });
    if (this.serverType.validator) {
      this.serverType.validator(ast);
    }
    let entitySets = (this.entitySets = odata.getPublicControllers(
      this.serverType
    ));
    this.events = [];
    this.workflow = [
      (body) => {
        ...
        let resourcePath = (this.resourcePath =
          new visitor_1.ResourcePathVisitor(
            this.serverType,
            this.entitySets
          ).Visit(ast));

        this.odataContext = createODataContext(
          context,
          entitySets,
          server,
          resourcePath,
            ...
            .map((part, i) => {
              ...
              switch (fn) {
                ...
                case "__EntityCollectionNavigationProperty":
                  return this.__EntityCollectionNavigationProperty.call(
                    this,
                    part
                  );
        ...
        try {
          ...
          if (chunk instanceof Object) {
            if (chunk["@odata.count"] || chunk.inlinecount) {
              this.streamInlineCount =
                chunk["@odata.count"] || chunk.inlinecount;
            ...
            let defer;
            if (this.ctrl)
              defer = this.__appendLinks(
                this.ctrl,
                this.elementType || this.ctrl.prototype.elementType,
                entity,
                chunk
              );
            let deferConvert = this.__convertEntity(
              entity,
              chunk,
              this.elementType || this.ctrl.prototype.elementType,
    ...
    return (result) => {
      ...
      try {
        ...
        let method =
          ...
          partIndex < this.resourcePath.navigation.length - 1
            ? "get"
            : this.method;
        let fn = odata.findODataMethod(
          this.ctrl,
        ...
        if (fn) {
          ...
          if (
            fnDesc.key.length == 1 &&
            part.key.length == 1 &&
            fnDesc.key[0].to != part.key[0].name
          ) {
          ...
          result.foreignKeys = {};
          let foreignFilter = Promise.all(
            foreignKeys.map((key) => {
              // tslib_1.__awaiter(this, void 0, void 0, function* () {
              result.foreignKeys[key] = result.body[typeKeys[0]];
              return `${key} eq ${Edm.escape(
                result.body[typeKeys[0]],
                Edm.getTypeName(elementType, key, this.serverType.container)
              )}`;
            })
          ).join(" and ");
          let params = {};
    ...
    return (result) => {
      ...
      try {
        ...
        if (fn) {
          ...
          this.elementType = elementType;
          return this.__read(ctrl, part, params, result, fn, elementType).then(
            (result) => {
              this.ctrl = this.serverType.getController(elementType);
              return result;
            }
        ...
        return new Promise((resolve, reject) => {
          ...
          let prevPart =
            this.resourcePath.navigation[
              this.resourcePath.navigation.indexOf(part) - 1
            ];
          let fn =
            odata.findODataMethod(
              this.ctrl,
              `${this.method}/${part.name}`,
              prevPart.key || []
            ) ||
            odata.findODataMethod(
              this.ctrl,
              `${this.method}/${part.name}/$value`,
              prevPart.key || []
            );
          if (!fn && this.method != "get") {
            ...
            if (fn) {
              ...
              this.body = {};
              this.body[part.name] =
                this.method == "delete" ? null : body.value || body;
            }
          }
          if (fn) {
          ...
          if (this.method == "get") {
            currentResult.then((value) => {
              try {
                result.body = {
                  "@odata.context":
                    this.options.metadata != ODataMetadataType.none
                      ? result.body["@odata.context"]
                      : undefined,
                  value: value,
                };
                let elementType = result.elementType;
                //if (value instanceof Object)
                result.elementType = Edm.isEnumType(
                  result.elementType,
                  part.name
                )
                  ? Edm.getTypeName(
                ...
                if (
                  value &&
                  (utils_1.isStream(value) || utils_1.isStream(value.stream))
                ) {
                  this.emit("header", {
                    "Content-Type":
                      Edm.getContentType(elementType.prototype, part.name) ||
                      value.contentType ||
  ...
  __read(ctrl, part, params, data, filter, elementType, include, select) {
    ...
    try {
      ...
      let method =
        writeMethods.indexOf(this.method) >= 0 &&
        this.resourcePath.navigation.indexOf(part) <
          this.resourcePath.navigation.length - 1
          ? "get"
          : this.method;
      ...
      if (typeof filter == "string" || !filter) {
        ...
        // if (!fn) return reject(new error_1.ResourceNotFoundError());
        if (!fn) return new error_1.ResourceNotFoundError();

        let queryString = filter
          ? `$filter=${filter}`
          : (include || this.url).query;
      ...
      switch (method) {
        ...
        case "patch":
          let body = data
            ? Object.assign(this.body || {}, data.foreignKeys)
            : this.body;
          ...
          if (typeParam) {
            params[typeParam] = (
              body["@odata.type"] ||
              `${ctrl.prototype.elementType.namespace}.${ctrl.prototype.elementType.name}`
            ).replace(/^#/, "");
          }
          if (bodyParam) {
            this.__deserialize(body, ctrl.prototype.elementType);
            this.__stripOData(body);
            params[bodyParam] = body;
          }
          if (!part.key) {
            let properties = Edm.getProperties(
              (elementType || ctrl.prototype.elementType).prototype
            );
            properties.forEach((prop) => {
              if (Edm.isKey(elementType || ctrl.prototype.elementType, prop)) {
                params[prop] =
                  (this.body || {})[prop] || ((data || {}).body || {})[prop];
              }
            });
          }
          currentResult = fnCaller.call(ctrl, fn, params);
      ...
      // return currentResult.then(
      {
        ...
        if (utils_1.isStream(result) && include) {
          ...
          utils_1.isStream(result) &&
          (!part.key ||
            !Edm.isMediaEntity(elementType || this.ctrl.prototype.elementType))
        ) {
          result.on("end", () => resolve(ODataRequestResult[method]()));
          result.on("error", reject);
        } else if (!(result instanceof result_1.ODataResult)) {
          result = ODataRequestResult[method](result);
          ...
          if (
            ...
            return new error_1.ResourceNotFoundError();
          // return reject(new error_1.ResourceNotFoundError());
          try {
            if (elementType) result.elementType = elementType;
            this.__appendODataContext(
              result,
              elementType || this.ctrl.prototype.elementType,
              (include || this.resourcePath).includes,
              select
            ...
            if (!this.streamEnd && this.streamEnabled && this.streamStart)
              this.on("end", () => result); //resolve(result));
            else return result;
            // }, reject);
          } catch (err) {
            throw err;
  ...
  __deserialize(obj, type) {
    ...
    for (let prop in obj) {
      try {
        ...
        if (typeof fn == "function") {
          obj[prop] = fn(obj[prop], prop, propType);
        } else if (typeof obj[prop] == "object") {
          this.__deserialize(obj[prop], propType);
        }
      } catch (err) {}
    }
    // });
      ...
      return new Promise((resolve, reject) => {
        ...
        try {
          ...
          if (
            ...
            schemas.some((schema) =>
              schema.entityContainer.some((container) =>
                container.actionImports.some(
                  (actionImport) => actionImport.name == part.name
                )
              )
            )
          ) {
          ...
          try {
            ...
            let entityBoundOp =
              typeof elementType == "function"
                ? elementType.prototype[boundOpName]
                : null;
            ...
            if (boundOp == expOp) {
              let expResult = Promise.resolve(
                boundOpName == "$count"
                  ? opResult || this.resultCount
                  : opResult
              );
              if (
                elementType &&
                boundOpName == "$value" &&
                typeof elementType == "function" &&
                Edm.isMediaEntity(elementType)
              ) {
                opResult.then((opResult) => {
                  if (this.method == "get") {
                    this.emit("header", {
                      "Content-Type":
                        Edm.getContentType(elementType) ||
                        opResult.contentType ||
                        "application/octet-stream",
                    });
                  ...
                  } else result_1.ODataResult.NoContent().then(resolve, reject);
                }, reject);
              } else {
                return expResult.then((expResult) => {
                  return (
                    boundOpName == "$ref" && this.method != "get"
                      ? result_1.ODataResult.NoContent
                      : ODataRequestResult[this.method]
                  )(
                    expResult,
                    typeof expResult == "object"
                      ? "application/json"
                      : "text/plain"
                  ).then((result) => {
                    if (
            ...
            if (isAction && !returnType) {
              return result_1.ODataResult.NoContent(opResult).then(
                resolve,
                reject
              );
            }
  ...
  __appendLinks(ctrl, elementType, context, body, result) {
    ...
    let entitySet =
      this.entitySets[this.resourcePath.navigation[0].name] == ctrl
        ? this.resourcePath.navigation[0].name
        : null;
    if (!entitySet) {
    ...
    let resolveBaseType = (elementType) => {
      if (elementType && elementType.prototype) {
        ...
        if (proto) {
          ...
          if (
            baseType != Object &&
            Edm.getProperties(baseType.prototype).length > 0
          ) {
    ...
    if (entitySet && typeof id != "undefined") {
      context["@odata.id"] = `${getODataRoot(
        this.context
      )}/${entitySet}(${id})`;
      if (typeof elementType == "function" && Edm.isMediaEntity(elementType)) {
        context["@odata.mediaReadLink"] = `${getODataRoot(
          this.context
        )}/${entitySet}(${id})/$value`;
        if (odata.findODataMethod(ctrl, "post/$value", [])) {
          context["@odata.mediaEditLink"] = `${getODataRoot(
            this.context
          )}/${entitySet}(${id})/$value`;
        }
      ...
      if (
        odata.findODataMethod(ctrl, "put", keys) ||
        odata.findODataMethod(ctrl, "patch", keys)
      ) {
        context["@odata.editLink"] = `${getODataRoot(
          this.context
        )}/${entitySet}(${id})`;
      }
    } else {
      if (typeof elementType == "function" && Edm.isMediaEntity(elementType)) {
        context["@odata.mediaReadLink"] = `${getODataRoot(this.context)}${
          this.context.url
        }(${id})/$value`;
        context["@odata.mediaReadLink"] = context[
          "@odata.mediaReadLink"
        ].replace(`(${id})(${id})`, `(${id})`);
        if (odata.findODataMethod(ctrl, "post/$value", [])) {
          context["@odata.mediaEditLink"] = `${getODataRoot(this.context)}${
            this.context.url
          }(${id})/$value`;
          context["@odata.mediaEditLink"] = context[
            "@odata.mediaEditLink"
          ].replace(`(${id})(${id})`, `(${id})`);
        }
        let contentType = Edm.getContentType(elementType);
      ...
      if (keys.length > 0 && typeof id != "undefined") {
        if (
          ...
          context["@odata.editLink"] = `${getODataRoot(this.context)}${
            this.context.url
          }(${id})`;
          context["@odata.editLink"] = context["@odata.editLink"].replace(
            `(${id})(${id})`,
            `(${id})`
          );
        }
  ...
  __appendODataContext(result, ctrlType, includes, select) {
    ...
    let elementType = (result.elementType =
      jsPrimitiveTypes.indexOf(result.elementType) >= 0 ||
      result.elementType == String ||
      typeof result.elementType != "function"
        ? ctrlType
        : result.elementType);
    if (typeof result.body == "object" && result.body) {
      ...
      if (!result.body["@odata.context"]) {
        let ctrl =
          this.ctrl && this.ctrl.prototype.elementType == ctrlType
            ? this.ctrl
            : this.serverType.getController(ctrlType);
  ...
  __setODataType(context, elementType) {
    ...
    if (containerType) {
      context["@odata.type"] = `#${
        odata.getNamespace(
          Object.getPrototypeOf(this.serverType.container).constructor,
          containerType
        ) ||
      ...
      context["@odata.type"] = `#${
        elementType.namespace || this.serverType.namespace
      }.${elementType.name}`;
    }
  }
  __convertEntity(context, result, elementType, includes, select) {
    // return tslib_1.__awaiter(this, void 0, void 0, function* () {
    if (
      !(elementType.prototype instanceof Object) ||
      elementType === Object ||
      this.options.disableEntityConversion
    )
      return Object.assign(context, result || {});
    let resultType = Object.getPrototypeOf(result).constructor;
    if (
      resultType != Object &&
      resultType != this.ctrl.prototype.elementType &&
      resultType.prototype instanceof this.ctrl.prototype.elementType
    ) {
      elementType = resultType;
      if (
        this.options.metadata != ODataMetadataType.none &&
        Edm.isEntityType(elementType)
      )
        ...
        props.map((prop) =>
          ((prop) => {
            ...
            if (
              ...
              !Edm.isTypeDefinition(elementType, prop)
            ) {
              itemType = function () {};
            ...
            if (
              ...
              if (isCollection && propValue) {
                let value = Array.isArray(propValue)
                  ? propValue
                  : typeof propValue != "undefined"
                  ? [propValue]
                  : [];
                for (let i = 0; i < value.length; i++) {
                  value[i] = this.__resolveAsync(
                    type,
                    prop,
                    value[i],
                    entity,
                ...
                if (this.options.metadata == ODataMetadataType.full) {
                  if (Edm.isEntityType(elementType, prop)) {
                    if (!includes || (includes && !includes[prop])) {
                      metadata[
                        `${prop}@odata.associationLink`
                      ] = `${context["@odata.id"]}/${prop}/$ref`;
                      metadata[
                        `${prop}@odata.navigationLink`
                      ] = `${context["@odata.id"]}/${prop}`;
                    }
                  } else if (type != "Edm.String" && type != "Edm.Boolean") {
                    let typeName = Edm.getTypeName(
                      elementType,
                      prop,
                      this.serverType.container
                    );
                ...
                if (includes && includes[prop]) {
                  ...
                  if (this.options.metadata != ODataMetadataType.none) {
                    context[
                      `${prop}@odata.mediaReadLink`
                    ] = `${context["@odata.id"]}/${prop}`;
                    if (
                      odata.findODataMethod(ctrl, `post/${prop}`, []) ||
                      odata.findODataMethod(ctrl, `post/${prop}/$value`, [])
                    ) {
                      context[
                        `${prop}@odata.mediaEditLink`
                      ] = `${context["@odata.id"]}/${prop}`;
                    }
                    let contentType =
                      Edm.getContentType(elementType.prototype, prop) ||
                      (propValue && propValue.contentType);
                    if (contentType)
  ...
  __include(include, select, context, prop, ctrl, result, elementType) {
    ...
    const isCollection = Edm.isCollection(
      elementType,
      include.navigationProperty
    );
    const navigationType = Edm.getType(
      elementType,
      include.navigationProperty,
      this.serverType.container
    );
    ...
    if (typeof result[prop] == "object") {
      navigationResult = result_1.ODataResult.Ok(result[prop]);
      this.__appendODataContext(
        navigationResult,
        navigationType,
        include.includes,
      ...
      const fn = odata.findODataMethod(
        ctrl,
        `get/${include.navigationProperty}`,
        []
      );
      let params = {};
      ...
      if (fn) {
        ...
        if (utils_1.isIterator(fnCall)) {
          fnResult = run(fnResult, defaultHandlers);
        }
        ...
        if (
          isCollection &&
          (utils_1.isStream(fnResult) ||
            !fnResult ||
            (stream && stream.buffer && stream.buffer.length > 0)) &&
          stream &&
          streamPromise
        )
          navigationResult = result_1.ODataResult.Ok(streamPromise || []);
        else navigationResult = result_1.ODataResult.Ok(fnResult);
        this.__appendODataContext(
          navigationResult,
          navigationType,
          include.includes,
        ...
        ctrl = this.serverType.getController(navigationType);
        if (isCollection) {
          let foreignKeys = Edm.getForeignKeys(
            elementType,
            include.navigationProperty
          );
          ...
          let foreignFilter = Promise.all(
            foreignKeys.map((key) => {
              ...
              return `${key} eq ${Edm.escape(
                result[typeKeys[0]],
                Edm.getTypeName(navigationType, key, this.serverType.container)
              )}`;
            })
          ).join(" and ");
          if (part.key)
            part.key.forEach((key) => (params[key.name] = key.value));
          navigationResult = this.__read(
            ctrl,
            part,
            params,
            result,
          ...
          const foreignKeys = Edm.getForeignKeys(
            elementType,
            include.navigationProperty
          );
          result.foreignKeys = {};
    ...
    if (
      isCollection &&
      navigationResult.body.value &&
      Array.isArray(navigationResult.body.value)
    ) {
      ...
      navigationResult.body &&
      Object.keys(navigationResult.body).length > 0
    ) {
  ...
  __enableStreaming(part) {
    this.streamEnabled =
      part ==
        this.resourcePath.navigation[this.resourcePath.navigation.length - 1] ||
      (this.resourcePath.navigation[
        this.resourcePath.navigation.indexOf(part) + 1
      ] &&
        this.resourcePath.navigation[
  ...
  __applyParams(container, name, params, queryString, result, include) {
    ...
    let elementType =
      ...
      (this.ctrl && this.ctrl.prototype.elementType) ||
      null;
    if (queryParam) {
      ...
      if (typeof queryAst == "string") {
        queryAst = this.serverType.parser.query(queryAst, {
          metadata:
            this.resourcePath.ast.metadata || this.serverType.$metadata().edmx,
        });
        if (!include)
          queryAst = deepmerge(
            queryAst,
        ...
        const lastNavigationPath =
          this.resourcePath.navigation[this.resourcePath.navigation.length - 1];
        const queryType =
          lastNavigationPath.type == "QualifiedEntityTypeName"
            ? this.resourcePath.navigation[
                this.resourcePath.navigation.length - 1
              ].node[visitor_1.ODATA_TYPE]
            : (result || this.ctrl.prototype).elementType;
        new visitor_1.ResourcePathVisitor(
          this.serverType,
          this.entitySets
        ).Visit(queryAst, {}, queryType);
      }
      params[queryParam] = this.serverType.connector
        ? this.serverType.connector.createQuery(queryAst, elementType)
        : queryAst;
    ...
    if (filterParam) {
      ...
      let resourceFilterAst =
        this.resourcePath.ast.value.query &&
        this.resourcePath.ast.value.query.value.options &&
        this.resourcePath.ast.value.query.value.options.find(
          (t) => t.type == TokenType.Filter
        );
      if (typeof filterAst == "string") {
        ...
        if (typeof filterAst == "string") {
          filterAst = this.serverType.parser.filter(filterAst, {
            metadata:
              this.resourcePath.ast.metadata ||
              this.serverType.$metadata().edmx,
          });
          const lastNavigationPath =
            this.resourcePath.navigation[
              this.resourcePath.navigation.length - 1
        ...
        filterAst =
          ...
          token.value &&
          token.value.options &&
          token.value.options.find((t) => t.type == TokenType.Filter);
      }
      if (filterAst && !include) {
        filterAst = deepmerge(filterAst, (resourceFilterAst || {}).value || {});
      }
      params[filterParam] = this.serverType.connector
        ? this.serverType.connector.createFilter(filterAst, elementType)
        : filterAst;
      if (container.prototype instanceof controller_1.ODataControllerBase) {
        const validator = container.validator;
        if (validator) {
    ...
    if (resultParam) {
      params[resultParam] =
        result instanceof result_1.ODataResult ? result.body : result;
    }
    if (idParam) {
      params[idParam] = decodeURI(
        this.resourcePath.id || this.body["@odata.id"]
      );
    }
  ...
  execute(body) {
    // return tslib_1.__awaiter(this, void 0, void 0, function* () {
    this.body = body;
    let next = this.workflow.shift().call(this, body);
    while (this.workflow.length > 0) {
      next = this.workflow.shift().call(this, next);
    }
    return next;
    // });
  }
