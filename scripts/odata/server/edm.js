  ...
  let decorator = function (target, targetKey, parameterIndex) {
    ...
    if (typeof parameterIndex == "number") {
      ...
      let existingParameters =
        Reflect.getOwnMetadata(EdmParameters, target, targetKey) || [];
      ...
      if (
        ...
        ((desc && typeof desc.value != "function") ||
          (!desc && typeof target[targetKey] != "function"))
      ) {
...
exports.GeographyMultiLineString = typeDecoratorFactory(
  "Edm.GeographyMultiLineString"
);
/** Edm.GeographyMultiPolygon primitive type property decorator */
exports.GeographyMultiPolygon = typeDecoratorFactory(
  ...
  return function (target, targetKey, parameterIndex) {
    if (typeof parameterIndex == "number") {
      ...
      if (typeof elementTypeName == "function") {
        elementTypeName =
          (elementTypeName.namespace || target.namespace) +
          "." +
...
function getTypeName(target, propertyKey, container) {
  ...
  type =
    (type && type["@odata.type"]) ||
    (type && type.prototype && type.prototype["@odata.type"]) ||
    type;
  if (container && container instanceof ContainerBase) {
    ...
    if (containerType) {
      ...
      let namespace =
        odata.getNamespace(containerPrototype.constructor, containerType) ||
        (containerPrototype.constructor || target).namespace ||
        "Default";
  ...
  if (
    ...
    if (type.__forward__ref__) {
      type =
        (type.namespace || target.namespace || "Default") + "." + type().name;
    } else
      type =
        (type.namespace || target.namespace || "Default") + "." + type.name;
  }
  if (
    ...
    if (elementType.__forward__ref__) {
      elementType =
        (elementType.namespace || target.namespace || "Default") +
        "." +
        elementType().name;
    } else
      elementType =
...
function getType(target, propertyKey, container) {
  let type = !propertyKey
    ? Reflect.getMetadata(EdmType, target)
    : Reflect.getMetadata(EdmType, target.prototype, propertyKey);
  let elementType = !propertyKey
    ? Reflect.getMetadata(EdmElementType, target)
    : Reflect.getMetadata(EdmElementType, target.prototype, propertyKey);
  let hasEntityType =
    isEntityType(target, propertyKey) || isComplexType(target, propertyKey);
  ...
  if (typeof elementType == "string") {
    if (container && container instanceof ContainerBase) {
      ...
      if (containerType) {
        let namespace =
          ...
          Object.getPrototypeOf(container).constructor.namespace ||
          target.namespace ||
          "Default";
        let containerTypeName = containerType.name;
    ...
    for (let i = 0; i < EdmContainer.length; i++) {
      ...
      let containerTypeName =
        (namespace ? namespace + "." : "") + containerType.name;
      if (containerTypeName == elementType) {
...
function isCollection(target, propertyKey) {
  return (
    Reflect.getMetadata(EdmType, target.prototype, propertyKey) == "Collection"
  );
}
...
function getParameters(target, targetKey) {
  ...
  while (__proto__) {
    params.push(
      ...(Reflect.getOwnMetadata(EdmParameters, __proto__, targetKey) || [])
    );
...
function isKey(target, propertyKey) {
  return (
    Reflect.getMetadata(EdmKeyProperty, target.prototype, propertyKey) || false
  );
}
exports.isKey = isKey;
...
function getKeyProperties(target) {
  return (
    Reflect.getOwnMetadata(EdmKeyProperties, target) ||
    Reflect.getOwnMetadata(EdmKeyProperties, target.prototype) ||
    []
  );
}
...
function escape(value, type, serializer) {
  ...
  if (typeof serializer == "function") return serializer(value);
...
function isComputed(target, propertyKey) {
  return (
    Reflect.getMetadata(EdmComputedProperty, target.prototype, propertyKey) ||
    false
  );
}
...
function operationDecoratorFactory(type, returnType) {
  return function (target, targetKey) {
    ...
    let existingOperations =
      Reflect.getOwnMetadata(EdmOperations, target) || [];
...
function getReturnTypeName(target, propertyKey, container) {
  ...
  return (
    getTypeName(returnType, EdmReturnType, container) ||
    getTypeName(target, propertyKey, container)
  );
}
...
function getReturnType(target, propertyKey, container) {
  ...
  return (
    getType(returnType, EdmReturnType, container) ||
    getType(target, propertyKey, container)
  );
}
...
function isFunctionImport(target, propertyKey) {
  return (
    Reflect.getMetadata(EdmFunction, target.prototype, propertyKey) || false
  );
}
...
function EntityType(type) {
  //在目标对象里target注入entityType
  //比如target为server,targetKey控件器的名称
  return function (target, targetKey, parameterIndex) {
    let baseType = Object.getPrototypeOf(target).constructor;
    if (baseType != Object && getProperties(baseType.prototype).length > 0) {
      EntityType()(baseType.prototype);
...
function isEntityType(target, propertyKey) {
  return typeof propertyKey == "string"
    ? !Reflect.hasMetadata(EdmComplexType, target.prototype, propertyKey) &&
        Reflect.hasMetadata(EdmEntityType, target.prototype, propertyKey)
    : !Reflect.hasMetadata(EdmComplexType, target) &&
        Reflect.hasMetadata(EdmEntityType, target);
}
...
function Annotate(...annotation) {
  return function (target, targetKey) {
    ...
    let existingAnnotations =
      Reflect.getOwnMetadata(EdmAnnotations, target, targetKey) || [];
...
function getAnnotations(target, targetKey) {
  try {
    return (
      Reflect.getOwnMetadata(EdmAnnotations, target.prototype, targetKey) ||
      Reflect.getOwnMetadata(EdmAnnotations, target, targetKey) ||
      []
    );
  } catch (_a) {
...
function ForeignKey(...keys) {
  return function (target, targetKey) {
    ...
    let existingForeignKeys =
      Reflect.getOwnMetadata(EdmForeignKeys, target, targetKey) || [];
...
function getForeignKeys(target, targetKey) {
  return (
    Reflect.getOwnMetadata(EdmForeignKeys, target.prototype, targetKey) ||
    Reflect.getOwnMetadata(EdmForeignKeys, target, targetKey) ||
    []
  );
}
...
function getPartner(target, targetKey) {
  return (
    Reflect.getMetadata(EdmPartnerProperty, target, targetKey) ||
    Reflect.getMetadata(EdmPartnerProperty, target.prototype, targetKey)
  );
}
