...
//yao run scripts.parser.v2.test.dummy
function dummy() {
  const queryObj = {
    $format: ["json"],
  ...
  const url1 = queryObjectToUrl(queryObj);
  console.log("url1:", url1);
  const result1 = odataParser.odataUri(url1);
  console.log(result1);

  const url2 = "$format=json&$skip=0&$top=10";
  console.log("url2:", url2);
  const result2 = odataParser.odataUri(url2);
  console.log(result2);
  const url =
    "$skip=0&$top=10&$select=browser,engine,id,platform,version&$skip=0&$top=10&expand=products/iu";
  const result = odataParser.odataUri(url);
  console.log(result);
}
